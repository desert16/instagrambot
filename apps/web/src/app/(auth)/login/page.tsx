...
import { Instagram, Lock, Mail, ArrowRight, Loader2 } from 'lucide-react';
import { fetchApi } from '@/lib/api-client';

export default function LoginPage() {
  const router = useRouter();
  const [email, setEmail] = useState('admin@instagrambot.local');
      ...
      <div className="max-w-md w-full p-8 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-2xl space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-xl font-bold text-white tracking-tight">
            Instagram AI SaaS Girişi
          </h1>
          <p className="text-xs text-slate-400">
            Hesabınıza giriş yaparak Instagram müşteri paneline erişin.
          </p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          >
            {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <span>Giriş Yap</span>}
